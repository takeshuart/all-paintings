...
async function fetchGoogleArtProjectFile() {
    ...
    const outputFile = path.join(DATA_FILES_ROOT, 'wiki/fulldata-Google_Art_Project_works_by_Vincent_van_Gogh.jsonl')
...
async function downloadVanGoghImageFile() {
    ...
    const file = path.join(DATA_FILES_ROOT, './wiki/fulldata-Google_Art_Project_works_by_Vincent_van_Gogh.jsonl')
...
async function crawlerWikimediaFileInfoFromPageIDs(pageIdsFile: string): Promise<any> {
    ...
    const file = path.join(DATA_FILES_ROOT, pageIdsFile)
...
async function fetchWikimediaCategory(category: string, continueFrom?: string): Promise<void> {
    ...
    try {
        ...
        const allArtworkOfAnC = path.join(DATA_FILES_ROOT, fileName)
